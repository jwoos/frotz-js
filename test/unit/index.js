...
const DFrotzInterface = require(path.join(__dirname, '../../index'));
...
describe('Class: DFrotzInterface', () => {
	...
	describe('Method: checkForSaveFile', () => {
		...
		beforeEach(() => {
			...
			spyOn(fs, 'stat').and.callFake((fileName, cb) => {
				if (fileName === 'a-bad-filename') {
					cb(true, null);
				} else {
					cb(false, {
						isFile: () => {
							return fileName !== 'is-directory';
						}
					});
				}
			});
		...
		it('should check file using stat', () => {
			frotz.checkForSaveFile();

			expect(fs.stat).toHaveBeenCalled();
		});

		it('should resolve to true if file exists', (done) => {
			frotz.saveFile = 'valid-file';
		...
		it('should resolve to false if file does not exist', (done) => {
			frotz.saveFile = 'a-bad-filename';
		...
		it('should reject if directory', () => {
			frotz.fileName = 'is-directory';
